...
const (
	packageName = "github.com/FluxNetworks/fluxpm"
)
...
func (Backend) Build() error {
	fmt.Println("compiling binary dist/fluxpm")
	return runWith(flagEnv(), "go", "build", "-ldflags", ldflags, "-tags", "prod", "-o", "dist/fluxpm", "cmd/fluxpm/main.go")
}
...
func (Backend) Test() error {
	fmt.Println("running fluxpm backend unit tests")
...
func Release() error {
	...
	version, ok := os.LookupEnv("FLUXPM_RELEASE_VERSION")
	if !ok {
		return errors.New("FLUXPM_RELEASE_VERSION must be set")
	}
	if !semverRegex.MatchString(version) {
		return errors.New("FLUXPM_RELEASE_VERSION must be a valid SemVer")
	}
	...
	err = sh.RunV("docker", "build", ".", "-t", "fluxpm/fluxpm:latest", "-t", "fluxpm/fluxpm:"+version)
	if err != nil {
		return err
	}
	err = sh.RunV("docker", "push", "fluxpm/fluxpm:latest")
	if err != nil {
		return err
	}
	err = sh.RunV("docker", "push", "fluxpm/fluxpm:"+version)
	if err != nil {
		return err
	}
...
// Up starts the docker-compose file using the `latest` fluxpm image
func (Latest) Up() error {
	return sh.RunV("docker-compose", "-p", "fluxpm-latest", "-f", "testing/docker-compose.latest.yml", "up")
}
...
func (Dev) Up() error {
	return sh.RunV("docker-compose", "-p", "fluxpm-dev", "-f", "testing/docker-compose.dev.yml", "up")
}
...
func (Docker) Up() error {
	return sh.RunV("docker-compose", "-p", "fluxpm", "up", "-d")
}

// Migrate runs the migration command for the docker-compose network
func (Docker) Migrate() error {
	return sh.RunV("docker-compose", "-p", "fluxpm", "-f", "docker-compose.yml", "-f", "docker-compose.migrate.yml", "run", "--rm", "migrate")
}
